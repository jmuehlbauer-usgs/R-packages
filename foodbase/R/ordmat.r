...
#' @details
#' This function will likely only work with \code{sampspec} set to an output generated by the companion \code{sampspec} function. If you ware working with data that do not have measured sizes and only counts (e.g., light trap data), then biomass and size matrices cannot be made and will return an error.
#'
...
#' foo4 <- ordmat(sampspec = foo3, type = "biomass")
...
if(type == 'count'){
	mat0[as.matrix(spec4[1:2])] <- sampspec$Specimens$CountTotal
	dat1 <- as.data.frame(mat0)
} else{	
	mat0[mat0 == 0] <- NA
	if(type == 'sizemean'){
		mat0[as.matrix(spec4[1:2])] <- sampspec$Specimens$SizeMean
		dat1 <- as.data.frame(mat0)
	}
	if(type == 'sizemed'){
		mat0[as.matrix(spec4[1:2])] <- sampspec$Specimens$SizeMedian
		dat1 <- as.data.frame(mat0)
	}
	if(type == 'sizesd'){
		mat0[as.matrix(spec4[1:2])] <- sampspec$Specimens$SizeSD
		dat1 <- as.data.frame(mat0)
	}
	if(type == 'biomass'){
		mat0[as.matrix(spec4[1:2])] <- sampspec$Biomass$BiomassTotal
		dat1 <- as.data.frame(mat0)
	}
}
